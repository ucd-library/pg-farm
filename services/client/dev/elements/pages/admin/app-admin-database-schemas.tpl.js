            ...
            ${(this.schemas || []).map(schema => html`
                <section>
                  ...
                  <div class='nav-buttons'>
                    <app-prefixed-icon-button icon='fa.solid.table' href='/db/${this.orgName}/${this.dbName}/edit/tables?schema=${schema.schema.name}'>
                      ${schema.schema.tableCount} Tables (${schema.publicTableCt} public)
                    </app-prefixed-icon-button>
                    <app-prefixed-icon-button icon='fa.solid.user' href='/db/${this.orgName}/${this.dbName}/edit/users?schema=${schema.schema.name}'>
                      ${schema.schema.userCount} Users ${schema.schema.isPublic ? '(1 public)' : ''}
                    </app-prefixed-icon-button>
                  </div>
                </section>
              `)}
