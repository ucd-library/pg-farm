...
class Database {
  ...
  async getDatabaseUsers(ctx) {
    ...
    const userColumns = [
      'u.username', 'u.user_id', 'type', 'u.first_name', 'u.last_name', 
      'u.ucd_iam_payload', 'sa.parent_user_id', 'sa.service_account_id'
    ];
      ...
      .map(row => {
        ...
        if( farmUser ) {
          obj.pgFarmUser = {
            ...
            serviceAccountId : farmUser.service_account_id
          };
        }
