...
class PgRest {
  ...
  async generateConfigFile(instNameOrId, orgNameOrId=null) {
    let user = await this.models.user.get(instNameOrId, orgNameOrId, config.pgRest.authenticator.username);
    ...
    let conStr = `postgresql://${escapedUser}:${escapedPass}@${user.instance_hostname}:${user.instance_port}/${user.database_name}`;
...
openapi-server-proxy-uri = "${config.appUrl}/api/db/${orgUrlName}/${user.database_name}"
