...
router.post('/',
  ...
  async (req, res) => {
  try {
    console.log("DEBUG CTX:", typeof req.context, req.context?.constructor?.name, JSON.stringify(req.context?.logSignal)); let result = await instanceModel.create(req.context, req.body);
